...
const ResumeCard = ({ title, count, background, text = 'text-gray-700' }: { title: string, count: number | string, background: string, text?: string }) => {
...
const FilterResumeContainer = () => {
    ...
    if (isPending) {
        return <>
            ...
            <ResumeCard background='bg-yellow-200' title="Chiffre d' affaires" count={0} />
            <ResumeCard background='bg-green-200' title="Chiffre d' affaires Taxables" count={0} />
            <ResumeCard background='bg-red-200' title="Chiffre d' affaires Exonorés" count={0} />
        </>
    }
    ...
    return <>
        <ResumeCard background='bg-primary text-white' text='text-white' title='Total Clients' count={store.items.length} />
        <ResumeCard background='bg-yellow-200' title="Chiffre d'affaires" count={formatNumberToFrenchStandard(data.results.total) ?? 0} />
        <ResumeCard background='bg-green-200' title="Chiffre d'affaires Taxables" count={formatNumberToFrenchStandard(data.results.taxable) ?? 0} />
        <ResumeCard background='bg-red-200' title="Chiffre d'affaires Exonorés" count={formatNumberToFrenchStandard(data.results.exo) ?? 0} />
    </>
}
