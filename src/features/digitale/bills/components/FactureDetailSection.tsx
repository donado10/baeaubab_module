...
import useGetEnterpriseFacture from '../api/use-get-entreprise-bls';
import { useParams, usePathname } from 'next/navigation';
import { IAgence, IDocumentFacture, IEntrepriseFacture } from '../interface';
...
const FactureListContainer = ({ onSetItemsBL, entreprise_id, month, year }: { onSetItemsBL: (items: IDocumentFacture[]) => void, entreprise_id: string, month: string; year: string }) => {

    const { data, isPending } = useGetEnterpriseFacture(entreprise_id.toString(), year, month)
    ...
    return <BonLivraisonList p_documents={data.result as IDocumentFacture[]} />
}
const BonLivraisonList = ({ p_documents }: { p_documents: IDocumentFacture[] }) => {
...
const FactureResume = ({ agence_dg, documentsBL, month, year }: { agence_dg: IAgence, documentsBL: IDocumentFacture[], month: string, year: string }) => {
...
const FactureDetailSectionContainer = () => {
    ...
    return <><FactureDetailSection agence={data.result} /></>
}

const FactureDetailSection = ({ agence }: { agence: IAgence }) => {
    const store = useEntrepriseFactureStore()
    const { entreprise_id } = useParams()
    ...
    const [nearEn, setNearEn] = useState<{ previous: IEntrepriseFacture | null; next: IEntrepriseFacture | null }>({ previous: null, next: null });
    useEffect(() => {
        ...
        <main className='flex  w-full min-h-screen border border-gray-500  overflow-scroll'>
            <div className='w-2/7 h-screen overflow-scroll border-r border-gray-500  '>
                ...
                <div className='h-[80vh] overflow-y-scroll'>
                    ...
                    <FactureListContainer onSetItemsBL={(values: IDocumentFacture[]) => store.setItemsBL(values)} month={store.periode[1]} year={store.periode[0]} entreprise_id={entreprise_id.toString()} />
                </div>
            </div>
            <div className='w-5/7 flex flex-col h-screen overflow-scroll'>
                ...
                {!store.selectedFacture && store.itemsBL.length > 0 && <FactureResume month={store.periode[1]} year={store.periode[0]} agence_dg={agence} documentsBL={store.itemsBL.filter((bl) => bl.entete.DO_Status != 2)} />}
...
export default FactureDetailSectionContainer